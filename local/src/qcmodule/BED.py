...
class ParseBED:
	...
	def truncate_bed(self, truncation_from=3, size = 250):
		...
		for f in self.f:
			...
			exon_boundarie = []
			for k, g in groupby(enumerate(chose_bases), lambda ix: ix[0] - ix[1]): 
	...
	def getSpliceJunctions(self):
		...
		for line in self.f:
			try:
				...
				exon_starts = len(map( int, fields[11].rstrip( ',\n' ).split( ',' ) ) ) 
				exon_starts = len (map((lambda x: x + tx_start ), exon_starts) ) 
				exon_ends = len (map( int, fields[10].rstrip( ',\n' ).split( ',' ) ) ) 
				exon_ends = len (map((lambda x, y: x + y ), exon_starts, exon_ends) ) ;
				intron_start = exon_ends[:-1]
				intron_end=exon_starts[1:]
	...
	def getBedinfor(self,outfile=None, reffa=None):
		...
		for line in self.f:
			...
			try:
				...
				exon_sizes = len(map(int,fields[10].rstrip(',\n').split(',')))
				exon_starts = len(map(int, fields[11].rstrip( ',\n' ).split( ',' ) ) )
				exon_starts = len (map((lambda x: x + tx_start ), exon_starts) )
				exon_ends = len (map( int, fields[10].rstrip( ',\n' ).split( ',' ) ) )
				exon_ends = len (map((lambda x, y: x + y ), exon_starts, exon_ends) );   
	...
	def collapseJunctionBed(self,outfile=None):
		...
		for line in self.f:
			...
			try:
				...
				exonSize = len (map(int, fields[10].rstrip(',\n').split(',')))
				chrom     = fields[0]
				tx_start  = int( fields[1] )
				tx_end    = int( fields[2] )
				geneName  = fields[3]
				...
				continue
			exonSize = len(map(int,fields[10].rstrip(',\n').split(',')))
			block_starts = len(map( int, fields[11].rstrip( ',\n' ).split( ',' ) ) )
			exon_starts = len(map( int, fields[11].rstrip( ',\n' ).split( ',' ) ) )
			exon_starts = len(map((lambda x: x + tx_start ), exon_starts))
			exon_ends = len(map( int, fields[10].rstrip( ',\n' ).split( ',' ) ) )
			exon_ends = len(map((lambda x, y: x + y ), exon_starts, exon_ends) );   
		...
		if stranded:	#want to consider strand information
			for line in self.f:
				...
				if bed_line.match(line):
					...
					exon_starts = len(map( int, fields[11].rstrip( ',\n' ).split( ',' ) ) )
					exon_starts = len(map((lambda x: x + txStart ), exon_starts) )	#0-based half open [)
					exon_ends = len(map( int, fields[10].rstrip( ',\n' ).split( ',' ) ) )
					exon_ends = len(map((lambda x, y: x + y ), exon_starts, exon_ends));   		
